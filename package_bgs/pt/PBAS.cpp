...
bool PBAS::process(cv::Mat* input, cv::Mat* output)
{
  ...
  init(input);
  std::cout << "	[[[ runs After  init(input);]]] " << runs << std::endl;
  ...
  {
	probMap_Pt	 = probMap.ptr<uchar>(j);
    ...
    for(int i = 0; i < resultMap->cols; ++i) 
    {
	  ...
	  *probMap_Pt = p;
	  //if(runs == N)
		 // std::cout << "\t" << *actualR_Pt << "\t" << *actualT_Pt;
      ...
      {
		  //if(runs == N)
			 // std::cout << "BG: " << count << "\t" << index << std::endl;
        *resultMap_Pt = 0;
      ...
      {
		  //if(runs == N)
			 // std::cout << "FG: " << count << "\t" << index << std::endl;
		  //if(runs == N && count == 0)
			 // getchar();
...
void PBAS::getProbMap(cv::Mat* prob)
{
	probMap.copyTo(*prob);
}
...
void PBAS::init(cv::Mat* input)
{
  if(runs < N)
  {
    ...
    if(runs == 0)
    {	
		probMap.create(input->size(), CV_8UC1);
